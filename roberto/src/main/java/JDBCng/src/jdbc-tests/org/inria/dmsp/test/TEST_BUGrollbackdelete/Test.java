package org.inria.dmsp.test.TEST_BUGrollbackdelete;
...
public class Test extends Tools implements ITest
{
	...
	public void run(PrintWriter out, String dbmsHost) throws Exception
	{
		...
		this.out = out; Tools_dmsp t = new Tools_dmsp(out);
...
//		select_star();
		...
		db.commit();
		if(perf==0){out.println("========== COMMIT DONE.======== ");}
		...
		if(perf==0){out.println(" =========== C O M M I T =========== ");}
		db.commit();
		...
		ts_spt = ((org.inria.jdbc.Connection) db).getGlobalTimestamp();
		if(perf==0){out.println(" --> call to getGlobalTimestamp, TS_SPT = "+ts_spt);}
		if(perf==0){out.println("-- TRY TO DELETE INFO with idG="+(IdGlobal_info-500));}
		res = Tools_dmsp.Test_DELETE_BY_ID((IdGlobal_info-500), ps);
		if(perf==0){out.println("--> delete return: "+res+" row(s) deleted...");}

		// SELECT THE INFO (AFTER DELETE):
		if(perf==0){out.println("-- AFTER DELETE : SELECT INFO with IdG="+(IdGlobal_info-500));}
		ps = ((org.inria.jdbc.Connection)db).prepareStatement(DMSP_QEP_IDs.EP_Synchro.EP_INFO_SELECT_BY_ID);
		lireResultSet(Tools_dmsp.Test_SELECT_BY_INT((IdGlobal_info-500), ps), out);
...
*/		
		...
		// scan the infos :
		if(perf==0){out.println("========== ALL INFOS BEFORE ROLLBACK ======== ");}
		st = db.createStatement();
		rs = ((org.inria.jdbc.Statement)st).executeQuery(DMSP_QEP_IDs.EP_TEST.EP_INFO_SELECT_STAR);
		lireResultSet(rs, out);
		if(perf==0){out.println("========== ALL INFOS END BEFORE ROLLBACK======== ");}
		// insert failure point: FUNC_DBITMAP_RECOVERY_FAIL, any table
	    Insert_Failure_Point(Macro.FUNC_DBITMAP_RECOVERY_FAIL, (byte)3, Macro.STRUCT_TYPE_TABLE);
	    // ROLLBACK:
	    try{
	    	db.rollback();
	    } catch(Exception e){
	    	if(perf==0){out.println("\nRollback Failure: FUNC_DBITMAP_RECOVERY_FAIL!");
...
/*		// generate a FP in update/delte list :
		...
		// scan the infos :
		if(perf==0){out.println("========== ALL INFOS AFTER ROLLBACK ======== ");}
		st = db.createStatement();
		rs = ((org.inria.jdbc.Statement)st).executeQuery(DMSP_QEP_IDs.EP_TEST.EP_INFO_SELECT_STAR);
		lireResultSet(rs, out);
