...
public class StoreFile 
{
	public static void storeFile(String fileToSendPath, User user ){
		...
		try {
			...
			StoreFileCommand storeCmd= new StoreFileCommand(Constants.CMD_STORE_FILE, Base64.getEncoder().encode(encrSkey).toString(), Base64.getEncoder().encode(iv).toString(), encryptFilePath, Base64.getEncoder().encode(encryptFile).toString());
