package org.inria.dmsp.test.TEST_BUGrollbackupdate;
...
public class Test extends Tools implements ITest
{
	...
	public void run(PrintWriter out, String dbmsHost) throws Exception
	{
         ...
         */
		if(perf==0){out.println("// ---------- TEST : UPDATE INFO AND ROLLBACK------------ ");}
		...
		// SELECT THE INFO (BEFORE UPDATE):
		if(perf==0){out.println("-- BEFORE UPDATE : SELECT INFO with IdG="+IdGlobal_info);}
		...
		int res = Tools_dmsp.Test_INFO_UPDATE( 
				ts_spt/*ts_spt*/,
				"NEW INFO1" /*valchar*/,
				1010101 /*valnum*/,
		...
		// SELECT THE INFO (AFTER UPDATE):
		if(perf==0){out.println("-- AFTER UPDATE : SELECT INFO with IdG="+IdGlobal_info);}
		ps = ((org.inria.jdbc.Connection)db).prepareStatement(DMSP_QEP_IDs.EP_Synchro.EP_INFO_SELECT_BY_ID);
		lireResultSet(Tools_dmsp.Test_SELECT_BY_INT(IdGlobal_info, ps), out);
		...
		// insert failure point: FUNC_UBITMAP_RECOVERY_FAIL, any table
	    Insert_Failure_Point(Macro.FUNC_UBITMAP_RECOVERY_FAIL, (byte)3, Macro.STRUCT_TYPE_TABLE);
	    // ROLLBACK:
	    try{
	    	db.rollback();
	    } catch(Exception e){
	    	if(perf==0){out.println("\nRollback Failure: FUNC_UBITMAP_RECOVERY_FAIL!");
	    	out.println("Restart DBMS and recover database.");}
	    }
		if(perf==0){out.println("=========== ROLLBACK DONE. ");}

		// SELECT THE INFO (AFTER ROLLBACK):
		if(perf==0){out.println("-- AFTER ROLLBACK : SELECT INFO with IdG="+IdGlobal_info);}
		ps = ((org.inria.jdbc.Connection)db).prepareStatement(DMSP_QEP_IDs.EP_Synchro.EP_INFO_SELECT_BY_ID);
		lireResultSet(Tools_dmsp.Test_SELECT_BY_INT(IdGlobal_info, ps), out);
